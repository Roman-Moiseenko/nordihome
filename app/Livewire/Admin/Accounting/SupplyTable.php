...
class SupplyTable extends DataTableComponent
{
    //protected $model = SupplyDocument::class;

    public bool $sortingPillsStatus = false;

    public function builder(): Builder
    {
        return SupplyDocument::query()->with(['staff:fullname,id']);
    }
    ...
    public function configure(): void
    {
        $this->setPrimaryKey('id')
            ...
            ->setTableRowUrl(function($row) {
                return route('admin.accounting.supply.show', $row->id);
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Кол-во товаров", 'id')->format(fn($value, $row, Column $column) => $row->getQuantity()),
            ...
            ButtonGroupColumn::make('Действия')->unclickable()->attributes(function($row) {
                ...
                ->title(fn($row) => IconHelper::trash() . ' Delete')->html()
                    ...
                    ->attributes(function($row) {
                        if ($row->isCreated()) {
                            return ModalDelete::attributes(route('admin.accounting.supply.destroy', $row));
                        } else {
    ...
    private function _status($row): string
    {
        ...
        $class = match ($row->status) {
            SupplyDocument::CREATED => 'py-1 px-2 rounded-full text-xs bg-secondary text-gray-800',
            SupplyDocument::SENT => 'py-1 px-2 rounded-full text-xs bg-warning text-white',
            SupplyDocument::COMPLETED => 'py-1 px-2 rounded-full text-xs bg-success text-white',
            default => '',
        };
