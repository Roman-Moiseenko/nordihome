<template>
    ...
    <el-config-provider :locale="ru">
        <h1 class="font-medium text-xl">Товары из категории Только под заказ</h1>
        <div class="flex mt-5">

            <SearchAddProduct
                :route="route('admin.product.on-order.add-product')"
            />
            <SearchAddProducts :route="route('admin.product.on-order.add-products')" class="ml-3"/>
...
const props = defineProps({
    ...
    title: {
        type: String,
        default: 'Товары из категории Только под заказ',
    },
    filters: Array,
    categories: Array,
})
...
function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.product.on-order.del-product', {product: row.id}));
}
