<template>
    ...
    <el-config-provider :locale="ru">
        <h1 class="font-medium text-xl">Сайт. Текстовые виджеты</h1>
        <div class="flex">
...
const props = defineProps({
    ...
    title: {
        type: String,
        default: 'Сайт. Текстовые виджеты',
    },
    templates: Array,
})
...
function copyBuffer(row) {
    navigator.clipboard.writeText('[text="' + row.id + '" name="' + row.name + '"]');
}
function saveWidget() {
    router.visit(route('admin.content.widget.text.store' ), {
        method: "post",
        data: form,
        preserveScroll: true,
        preserveState: true,
...
function onToggle(row) {
    router.visit(route('admin.content.widget.text.toggle', {widget: row.id}), {
        method: "post",
        preserveScroll: true,
        preserveState: false,
    })
...
function routeClick(row) {
   router.get(route('admin.content.widget.text.show', {widget: row.id}))
}

function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.content.widget.text.destroy', {widget: row.id}));
}
</script>
