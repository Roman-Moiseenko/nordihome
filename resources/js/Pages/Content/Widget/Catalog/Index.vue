<template>
    ...
    <el-config-provider :locale="ru">
        <h1 class="font-medium text-xl">Сайт. Каталог (Виджет)</h1>
        <div class="flex">
        ...
        <DeleteEntityModal name_entity="Каталог"/>


        <el-dialog v-model="dialogCreate" title="Каталог" width="500">
...
function copyBuffer(row) {
    navigator.clipboard.writeText('[catalog="' + row.id + '" name="' + row.name + '"]');
}
...
function saveWidget() {
    router.post(route('admin.content.widget.catalog.store' ), form)
}

function routeClick(row) {

   router.get(route('admin.content.widget.catalog.show', {widget: row.id}))
}

function onToggle(row) {
    router.visit(route('admin.content.widget.catalog.toggle', {widget: row.id}), {
        method: "post",
        preserveScroll: true,
        preserveState: false,
    })
...
function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.content.widget.catalog.destroy', {widget: row.id}));
}
</script>
