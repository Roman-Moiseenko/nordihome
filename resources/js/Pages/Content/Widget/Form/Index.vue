<template>
    ...
    <el-config-provider :locale="ru">
        <h1 class="font-medium text-xl">Сайт. Формы обратной связи</h1>
        <div class="flex">
        ...
        <div class="mt-2 p-5 bg-white rounded-md">
            ...
            >
                <el-table-column prop="name" label="Форма" width="280" show-overflow-tooltip/>
...
const props = defineProps({
    ...
    title: {
        type: String,
        default: 'Сайт. Формы обратной связи',
    },
    templates: Array,
})
...
function copyBuffer(row) {
    navigator.clipboard.writeText('[form="' + row.id + '" name="' + row.name + '"]');
}
function saveWidget() {
    router.visit(route('admin.content.widget.form.store' ), {
        method: "post",
        data: form,
        preserveScroll: true,
        preserveState: true,
...
function onToggle(row) {
    router.visit(route('admin.content.widget.form.toggle', {widget: row.id}), {
        method: "post",
        preserveScroll: true,
        preserveState: false,
    })
...
function routeClick(row) {
   router.get(route('admin.content.widget.form.show', {widget: row.id}))
}

function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.content.widget.form.destroy', {widget: row.id}));
}
</script>
