<template>
    ...
    <el-config-provider :locale="ru">
        ...
        <div class="flex mt-5">
            ...
            <SearchAddProduct
                :route="route('admin.catalog.on-order.add-product')"
            />
            <SearchAddProducts :route="route('admin.catalog.on-order.add-products')" class="ml-3"/>
            <TableFilter :filter="filter" class="ml-auto" :count="filters.count">

                <el-select filterable v-model="filter.room" placeholder="Категория" class="mt-1">
                    <el-option v-for="item in useCatalog.categories" :key="item.id" :label="item.name"
                               :value="item.id"/>
                </el-select>
