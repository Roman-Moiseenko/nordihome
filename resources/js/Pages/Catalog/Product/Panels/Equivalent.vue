                    ...
                    <div v-if="product.equivalent">
                        ...
                        <div v-for="equi_prod in product.equivalent.products" class="mt-3 flex items-center p-2 bg-slate-100 rounded-md">
                            ...
                            <span class="font-medium ml-2">
                                ...
                                <Link v-else type="primary" :href="route('admin.catalog.product.edit', {product: equi_prod.id})">{{ equi_prod.name }}</Link>
                            </span>
                        </div>
                    </div>
                    <!-- Повторить -->
...
function onSave() {
    isSaving.value = true;
    router.visit(route('admin.catalog.product.edit.equivalent', {product: props.product.id}), {
        method: "post",
        data: form,
        preserveState: true,
        preserveScroll: true,
