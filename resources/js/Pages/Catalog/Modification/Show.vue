    ...
    <el-config-provider :locale="ru">
        ...
        <div v-for="product in modification.products" class="p-3 bg-white rounded-md items-center flex mb-1 p-2">
            ...
            <div class="ml-4" style="min-width: 350px;">
                <Link type="primary" :href="route('admin.catalog.product.edit', {product: product.id})">{{ product.name }}</Link>
            </div>
            <div>
...
function onBase(product) {
    router.visit(route('admin.catalog.modification.set-base', {modification: props.modification.id}), {
        method: "post",
        data: {
...
function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.catalog.modification.del-product', {modification: props.modification.id, product_id: row.id}));
}
function handleGetProduct(val) {
    router.visit(route('admin.catalog.modification.add-product', {modification: props.modification.id}), {
        method: "post",
        data: {
            product_id: val
        },
