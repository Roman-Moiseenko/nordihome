    ...
    <Card title="Акции">
        <el-row>
            <el-col :span="12" class="bg-green-50 p-2">
                <div v-for="item in useCatalog.promotions" :key="item.id" class="py-1">
                    <el-checkbox
                        :model-value="feed.promotionsIn.includes(item.id)"
            ...
            <el-col :span="12" class="bg-red-50 p-2">
                <div v-for="item in useCatalog.promotions" :key="item.id" class="py-1">
                    <el-checkbox
                        :model-value="feed.promotionsOut.includes(item.id)"
...
function toggle(id: number, checked: boolean, _in: boolean) {
    props.save('promotions', checked ? 'add' : 'remove', _in, [id])
}
</script>
