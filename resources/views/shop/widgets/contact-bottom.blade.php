...
<div class="mob-bottom-open">
    <div class="mob-bottom-social">
        @if(isset($contacts['mail']))
            <a href="{{ $contacts['mail']->url }}" target="_blank">{!! $contacts['mail']->svg !!}</a>
        @endif
        @if(isset($contacts['vk_1']))
            <a href="{{ $contacts['vk_1']->url }}" target="_blank">{!! $contacts['vk_1']->svg !!}</a>
        @endif
        ...
        @if(isset($contacts['telegram_bot']))
            <a href="{{ $contacts['telegram_bot']->url }}" target="_blank">{!! $contacts['telegram_bot']->svg !!}</a>
        @endif
        @if(isset($contacts['max_bot']))
            <a href="{{ $contacts['max_bot']->url }}" data-type="8" target="_blank">{!! $contacts['max_bot']->svg !!}</a>
        @endif
    </div>
</div>
