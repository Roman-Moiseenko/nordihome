...
<div class="mob-bottom-open">
    <div class="mob-bottom-social">
        @if(isset($contacts['mail']))
            <a href="{{ $contacts['mail']->url }}" target="_blank"
               data-analytics-action="contact_click"
               data-analytics-payload='{"channel":"{{ $contacts['mail']->channel }}","placement":"header"}'
            >{!! $contacts['mail']->svg !!}</a>
        @endif
        @if(isset($contacts['vk_1']))
            <a href="{{ $contacts['vk_1']->url }}" target="_blank"
               data-analytics-action="contact_click"
               data-analytics-payload='{"channel":"{{ $contacts['vk_1']->channel }}","placement":"header"}'
            >{!! $contacts['vk_1']->svg !!}</a>
        @endif
        ...
        @if(isset($contacts['telegram_bot']))
            <a href="{{ $contacts['telegram_bot']->url }}" target="_blank"
               data-analytics-action="contact_click"
               data-analytics-payload='{"channel":"{{ $contacts['telegram_bot']->channel }}","placement":"header"}'
            >{!! $contacts['telegram_bot']->svg !!}</a>
        @endif
        @if(isset($contacts['max_bot']))
            <a href="{{ $contacts['max_bot']->url }}" data-type="8" target="_blank"
               data-analytics-action="contact_click"
               data-analytics-payload='{"channel":"{{ $contacts['max_bot']->channel }}","placement":"header"}'
