<form method="post" action="{{ route('admin.sales.order.add-item', $order) }}">
    ...
    <div class="mx-3 flex w-full mb-5">
        ...
        <x-base.button id="add-product" type="button" variant="primary" class="ml-3" data-route="">Добавить товар в
            документ
        </x-base.button>
    </div>
</form>
...
@foreach($order->getInStock() as $i => $item)
    <livewire:admin.sales.order.manager-item :item="$item" :i="$i" :edit="$order->isManager()"/>
@endforeach
...
<script>
    ...
    addProductButton.addEventListener('click', function () {
        Livewire.dispatch(
            'add-product',
            {
                product_id: document.getElementById('hidden-id').value,
