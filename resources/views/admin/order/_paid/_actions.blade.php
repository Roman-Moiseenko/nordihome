...
@if($order->getQuantity() > $order->getQuantityExpense())
    <x-base.popover class="inline-block mt-auto ml-3" placement="bottom-start">
        ...
        <x-base.popover.panel>
            <form action="{{ route('admin.order.movement', $order) }}" METHOD="POST">
                @csrf
                <div class="p-2">
...
@if($order->isPrepaid())
    <div class="ml-auto flex flex-nowrap">
        ...
        <form id="form-send-invoice" method="post" action="{{ route('admin.order.send-invoice', $order) }}">
            @csrf
        </form>
        ...
        <form id="form-resend-invoice" method="post"
              action="{{ route('admin.order.resend-invoice', $order) }}">
            @csrf
        </form>
    </div>
@endif
