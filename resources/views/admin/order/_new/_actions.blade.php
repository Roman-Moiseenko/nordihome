@if($admin->isAdmin() || $admin->isChief())
    <x-base.popover class="inline-block mt-auto" placement="bottom-start">
        ...
        <x-base.popover.panel>
            <form action="{{ route('admin.order.set-manager', $order) }}" METHOD="POST">
                @csrf
                <div class="p-2">
...
@endif
@if($admin->isStaff())
    <button class="btn btn-primary ml-2" onclick="document.getElementById('form-order-take').submit();">Взять заказ
    </button>
    <form id="form-order-take" method="post" action="{{ route('admin.order.take', $order) }}">
        @csrf
    </form>
@endif
<button class="btn btn-secondary ml-2" onclick="document.getElementById('form-order-delete').submit();">Удалить</button>
<form id="form-order-delete" method="post" action="{{ route('admin.order.destroy', $order) }}">
    @method('DELETE')
    @csrf
</form>
