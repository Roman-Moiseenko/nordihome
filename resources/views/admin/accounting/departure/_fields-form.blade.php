            ...
            <div class="p-5">
                <div class="grid grid-cols-12 gap-2">
                    <div class="col-span-12 lg:col-span-6">
                        {{ \App\Forms\Input::create('number', ['placeholder' => 'Номер', 'value' => $departure->number ?? '', 'class' => 'mt-5'])->show() }}

                        <x-base.form-label for="select-storage-id" class="mt-3">Хранилище списания</x-base.form-label>
                        <x-base.tom-select id="select-storage-id" name="storage_id" class="w-full" data-placeholder="Выберите хранилище">
                            <option value="0"></option>
                            @foreach($storages as $storage)
                                <option value="{{ $storage->id }}"
                                @if($departure)
                                    {{ $departure->storage_id == $storage->id ? 'selected' : ''}}
                                    @endif
                                >
                                    {{ $storage->name }}
                                </option>
                        ...
                        {{ \App\Forms\Input::create('comment', ['placeholder' => 'Комментарий', 'value' => $departure->comment ?? '', 'class' => 'mt-3'])->label('Комментарий')->show() }}
